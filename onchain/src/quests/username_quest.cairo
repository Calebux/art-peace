...
#[abi(embed_v0)]
    impl UsernameQuest of IQuest<ContractState> {
        ...
        fn is_claimable(
            ...
            let claim_username_address = username_store_main.get_username(self.username.read());

            if claim_username_address == user {
                return true;
            }
        ...
        fn claim(ref self: ContractState, user: ContractAddress, calldata: Span<felt252>) -> u32 {
            assert(
                get_caller_address() == self.username_store.read(), 'Only Username can claim quests'
            );
